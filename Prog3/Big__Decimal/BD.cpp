...
Big_Decimal Big_Decimal::Subtraction(Big_Decimal& bd1, Big_Decimal& bd2, bool& overflow) {
	Big_Decimal tmp1 = bd1, tmp2 = bd2, res;
	tmp2.value[0] = 48 + '9' - tmp2.value[0]; // знак у второго меняется 
	res = Addition(tmp1, tmp2, overflow);
	return res;
}
Big_Decimal Big_Decimal::Increase(Big_Decimal& bd, bool& overflow) {
	int i;
	for (i = 1; ((i < N) && (bd.value[i] == '0')); i++);// находим индекс первого ненулевого (типа чтоб нули не двигать)
...
Big_Decimal Big_Decimal::Long_To_Big_Decimal(long int l, bool& overflow) {
	long int tmp = l;
	if (l < 0) {
...
std::istream& Big_Decimal::Input(std::istream& s, Big_Decimal& bd, bool& overflow) {
	char* str = getstr(s);
	bd = String_To_Big_Decimal(str, overflow);
	return s;
}

std::ostream& Big_Decimal::Show(std::ostream& s) {
	if (value[0] == '9') s << "-";
	int i;
	for (i = 1; ((i < N) && (value[i] == '0')); i++); // индекс первого ненулевого
	if (i == N) { s << '0'; }
	for (int j = i; j < N; j++) {
		s << value[j];
	}
